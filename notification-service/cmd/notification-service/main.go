...
func main() {
	...
	if providerMode == "REAL" {
		sender = email.NewMockEmailSender(0.0) 
	} else {
		sender = email.NewMockEmailSender(0.3)
	}

	consumer, err := rabbitmq.NewConsumer(rabbitMQURL, exchangeName, queueName, dlxName, dlqName, rdb, sender)
	if err != nil {
